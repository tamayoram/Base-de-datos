...
/* Eliminar el registro con lugar_nac Bogotá. Sin embargo, es necesario eliminar los registros en otras tablas dadas las relaciones establecidas */
...
where us_id in 
 (select us_id from [dbo].[usuario]
where lugar_nac='Bogotá');
go

select * from [dbo].[experiencia];

delete [dbo].[experiencia]
where us_id in 
 (select us_id from [dbo].[usuario]
where lugar_nac='Bogotá');
go
...
/* Eliminar todos los registros de la tabla experiencia*/

delete from [dbo].[experiencia];

select*from [dbo].[experiencia];

/* Eliminar registro de la tabla datosComunes*/

select*from [dbo].[datosComunes];

delete from [dbo].[datosComunes]
where relacion<10;
go
